...
\begin{document}
	...
	\begin{frame}{\textbf{Peres-Horodecki Criterion}}
		\begin{itemize}
			...
			\item \onslide<4->{Calculate the eigenvalues of $\rho^{T_B}$}.
			\item \onslide<5->{if at least one eigenvalue < 0 , this will ensure that the two subsystems are entangled}
			\item \onslide<6->{if all eigenvalues are > 0, then, in 2×2 or 2×3 systems, this implies the state is separable.}
		\end{itemize}
	\end{frame}
	\begin{frame}
		\begin{center}
	...
	\begin{frame}{\textbf{\large Polynomial Approach to Entanglement}}
		\begin{itemize}
			...
			\item \onslide<4->{Product of the variables is associated with a link between them.}
