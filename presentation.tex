...
\begin{document}
	...
	\begin{frame}{\textbf{\large Introduction to Quantum Information}}
		\onslide<2->{A pure state is a quantum state that can be described by a single state vector in a Hilbert space}
		\onslide<3->{Suppose a quantum system is in one of several states $\ket{\psi}$, we do not have complete knowledge about a quantum state, i.e when the state is in statistical ensemble of many pure states where i is an index, with respective probabilities $p_i$. The set $p_i$, $\ket{\psi
		_i}$ is called an ensemble of
		pure states. We can not represent such a state as a single vector in the Hilbert space. We represent such states using density operator.
		A mixed state is represented by a density matrix:
		\[
		\rho = \sum_i p_i \ket{\psi_i}\bra{\psi_i}
		\]
		where \( p_i \geq 0 \), \( \sum_i p_i = 1 \), and \( \ket{\psi_i} \) are pure states.}
	\end{frame}
