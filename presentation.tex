...
\begin{document}
	...
	\begin{frame}{\textbf{How it all began $\ldots$}}
		...
		Here $\lambda_i$ are the square root of the eigenvalues, in decreasing order, matrix $\tilde{\rho}\rho$ where $\tilde{\rho}$ is defined by:
	...
	\begin{frame}{\textbf{Polynomial Approach to Entanglement}}
		...
		\end{itemize}
		\begin{figure}[H]
			\onslide<8->{\scalebox{0.6}{\input{pics/2_1.tex}}}
		\end{figure}
	\end{frame}
	\begin{frame}{\textbf{Polynomial Approach to Entanglement}}
		\begin{itemize}
	...
	\begin{frame}{\textbf{Obtaining State from a given Link}}
		...
		\begin{enumerate}
			...
			\begin{itemize}
				...
				\onslide<11->{\item $\ket{\mathrm{Q_d}}\longrightarrow$  qudit state associated with \textit{artifically} introduced ring variable (alphabetical successor of the largest ring variable present). To be traced out later, hence is of less significance.}
			\end{itemize}
			\item \onslide<12->{Full state $\ket{\psi}\longrightarrow$ sum of individual such states. Trace out $d$ and obtain density matrix of reduced system. This becomes the density matrix of a \textbf{mixed state}.}
		\end{enumerate}
	...
	\begin{frame}{\textbf{Demonstration of Algorithm}}
		...
		\begin{frame}{Demonstration of Algorithm}
			...
			\begin{itemize}
				\item \onslide<1->{\textbf{Full state characterising the link}: $ \ket{\psi} = c_1\ket{\psi_1} + c_2\ket{\psi_2}$}
	...
	\begin{frame}{\textbf{Four Qubit System}}
		...
		\begin{figure}
			\centering
			\scalebox{0.5}{\input{pics/4_20.tex}}
		\end{figure}
		...
		\begin{frame}{\textbf{Four Qubit System}}
			...
			Consider, 
			\begin{equation*}
				\left[\begin{matrix}0\\0\\1.0\\0\\0\\0\\0\\0\end{matrix}\right] = \ket{0}_a\ket{1}_c\ket{0}_d.
			\end{equation*}
