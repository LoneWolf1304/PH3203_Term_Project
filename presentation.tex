...
\begin{document}
	...
	\begin{frame}{\textbf{\large Introduction to Quantum Information}
		\begin{itemize}
			\item \onslide<2->{A pure state is a quantum state that can be described by a single state vector in a Hilbert space}
			\item \onslide<3->{when a system is in statistical ensemble of many pure states $\ket{\psi_i}$ (where i is an index), with respective probabilities $p_i$, such a state is called mixed states. We represent such states using density operator: $\rho = \sum_i p_i \ket{\psi_i}\bra{\psi_i}$, where \( p_i \geq 0 \), \( \sum_i p_i = 1 \), and \( \ket{\psi_i} \) are pure states.}
			\item \onslide<4->{If one of the $p_i$} = 1, the state is a pure state, with the properties $\rho^2$ = $\rho$, $\mathrm{Tr}(\rho^2)$ = 1
			\item \onslide<5->{A quantum state \( \rho_{AB} \) is \textbf{separable} if it can be written as: $\rho_{AB} = \sum_i p_i$ , $\rho_A^{(i)} \otimes \rho_B^{(i)}$ where $\( p_i \geq 0 \)$, $\( \sum_i p_i = 1 \)$, and $\( \rho_A^{(i)} \)$ and $\( \rho_B^{(i)} \)$ are density matrices of subsystems A and B respectively.}
			\item \onslide<6->{A state is entangled if it is not separable.}
		\end{itemize}
	\end{frame}
	...
	\begin{frame}{\textbf{How it all began $\ldots$}}
		\begin{itemize}
			...
			\item \onslide<3->{Used \textit{Concurrence Test} to determine separability of the state}
		\end{itemize}
	...
	\end{frame}
	\begin{frame}{\textbf{\large Polynomial Approach to Entanglement}}
	...
	\begin{frame}{Demonstration of Algorithm}
		...
		\begin{frame}{Demonstration of Algorithm}
			...
			\begin{figure}
				\centering 
				\scalebox{0.6}{\input{pics/algo_pic.tex}}
	...
	\begin{frame}{\textbf{Three Qubit System}: 3\textsuperscript{1} class}
		...
		\onslide<2->{\begin{minipage}{0.35\textwidth}
			%\input{pics/borromean.tex}  % Make sure this file exists
			\scalebox{0.6}{\input{pics/borromean.tex}}
			\begin{itemize}
	...
	\begin{frame}{\textbf{Three Qubit System}: 3\textsuperscript{1} class}
		...
		\begin{minipage}{0.35\textwidth}
			%\input{pics/borromean.tex}  % Make sure this file exists
			\onslide<2->{\scalebox{0.6}{\input{pics/borromean_cut.tex}}}
			\begin{itemize}
	...
	\begin{frame}{\textbf{Three Qubit System}: 3\textsuperscript{2} class}
		...
		\begin{minipage}{0.35\textwidth}
			%\input{pics/borromean.tex}  % Make sure this file exists
			\begin{figure}
				\onslide<2->{\scalebox{0.4}{\input{pics/3_2.tex}}\end{figure}}
			\begin{itemize}
	...
	\begin{frame}{\textbf{Three Qubit System}: 3\textsuperscript{2} class: cut}
		...
		\begin{minipage}{0.35\textwidth}
			%\input{pics/borromean.tex}  % Make sure this file exists
			\begin{figure}
				\onslide<2->{\scalebox{0.4}{\input{pics/3_2_cut1.tex}}}\end{figure}
			\begin{itemize}
	...
	\begin{frame}{\textbf{Three Qubit System}: 3\textsuperscript{2} class: another cut}
		...
		\begin{minipage}{0.35\textwidth}
			%\input{pics/borromean.tex}  % Make sure this file exists
			\begin{figure}
				\onslide<2->{\scalebox{0.4}{\input{pics/3_2_cut2.tex}}}\end{figure}
			\begin{itemize}
